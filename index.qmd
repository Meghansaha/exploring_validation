...
<br><br><br> **Date/Time/Object Metadata**
...
<br><br> **`STEP`: The name of the validation functions used. Color-coded tabs let us know if a step was completed. Darker green means everything in the step passed**
...
<br><br><br> **`COLUMNS`: The target columns we told the agent to interrogate via our validation rules**
...
<br><br> **`VALUES`: Any required values needed/used to test for validation if applicable.**
...
<br><br> **`TBL`: Let's us know if the table was mutated in a validation step.** <br><br> **`EVAL`: Let's us know if there's issues R might have evaluating the table itself.**
...
<br><br><br> **`PASS`/`FAIL`: Gives the number/percentage of passing and failing unit tests**
...
<br><br> **`W,S,N`: Tells us if the validation steps have entered `WARN`, `STOP`, or `NOTIFY`. This is empty because there's no `action levels` set.**
...
<br><br><br> **`EXT`: Provides a download of a data extract of observations that failed any validations if applicable.**
...
Fails will [**halt**]{style="color:red;"} R's workflow
...
Passes will [**maintain**]{style="color:green;"} R's workflow
